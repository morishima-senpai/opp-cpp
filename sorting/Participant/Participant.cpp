...
int Participant::get_solved() const {
    return _solved;
}
int Participant::get_time_spent() const {
    return _time_spent;
}
float Participant::get_score() const {
    return _score;
}
int Participant::get_wrong_attempts() const {
    return _wrong_attempts;
}


void Participant::set_id(int id) {
    _id = id;
}
void Participant::set_solved(int solved) {
    _solved = solved;
}
void Participant::set_time_spent(int time_spent) {
    _time_spent = time_spent;
}
void Participant::set_score(float score) {
    _score = score;
}
void Participant::set_wrong_attempts(int wrong_attempts) {
    _wrong_attempts = wrong_attempts;
}
...
bool Participant::operator<(const Participant& other) const {
    ...
    if (_solved != other._solved) {
        return _solved > other._solved;
    }

    if (this->get_time_spent() != other.get_time_spent()) {
        return this->get_time_spent() < other.get_time_spent();
    }

    if (this->get_wrong_attempts() != other.get_wrong_attempts()) {
        return this->get_wrong_attempts() < other.get_wrong_attempts();
    }

    return this->get_id() < other.get_id(); // Надеюсь, это сработает
}

// std::ostream &operator<<(std::ostream &out, const Participant &par){
//     out
        // << std::left
        // << std::setw(10) << par.get_id()
        // << std::setw(15) << par.get_solved()
        // << std::setw(10) << par.get_time_spent()
        // << std::setw(10) << par.get_score()
        // << std::setw(10) << par.get_wrong_attempts();
