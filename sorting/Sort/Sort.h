...
private:
    ...
    // T_max = O(logN), T_min = O(1), T_mean = ? (see lectures)
    static void heap_sort_helper(T** arr, size_t n, size_t i, long long& comparisons) {
        ...
        if (largest != i) {
            ...
            heap_sort_helper(arr, n, largest /*= i*2+1or2 */, comparisons);
        }
    }
...
public:
    ...
    static long long heap_sort(T** arr, size_t n) {
        ...
        for (size_t i = n - 1; i > 0; i--) {  // T = O(N)
            std::swap(arr[0], arr[i]);
            heap_sort_helper(arr, i, 0, comparisons);  // T_max = O(logN)
        }
